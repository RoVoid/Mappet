...
public class ScriptWorld {
    ...
    public void setBlock(ScriptBlockState block, int x, int y, int z) {
        if (isBlockLoaded(x, y, z)) world.setBlockState(pos, block.asMinecraft(), 2 | 4);
    }

    public void setBlock(ScriptBlockState block, ScriptVector pos) {
    ...
    public ScriptBlockState getBlock(int x, int y, int z) {
        return isBlockLoaded(x, y, z) ? ScriptBlockState.create(world.getBlockState(pos).getActualState(world, pos)) : ScriptBlockState.AIR;
    }

    public ScriptBlockState getBlock(ScriptVector pos) {
    ...
    public void setTileEntity(ScriptBlockState block, int x, int y, int z, ScriptNBTCompound tileData) {
    ...
    public void setTileEntity(ScriptBlockState block, ScriptVector pos, ScriptNBTCompound tileData) {
    ...
    public ScriptTileEntity getTileEntity(ScriptVector pos) {
    ...
    public void replaceBlocks(ScriptBlockState block, ScriptBlockState newBlock, ScriptBox box) {
    ...
    public void replaceBlocks(ScriptBlockState block, ScriptBlockState newBlock, ScriptNBTCompound tileData, ScriptVector start, ScriptVector end) {
        replaceBlocks(block, newBlock, tileData, new ScriptBox(start.x, start.y, start.z, end.x, end.y, end.z));
    }

    public void replaceBlocks(ScriptBlockState block, ScriptBlockState newBlock, ScriptNBTCompound tileData, ScriptBox box) {
        for (int x = (int) Math.floor(box.minX); x <= (int) box.maxX; x++) {
            for (int y = (int) Math.floor(box.minY); y <= (int) box.maxY; y++) {
                for (int z = (int) Math.floor(box.minZ); z <= (int) box.maxZ; z++) {
    ...
    public ScriptNpc spawnNpc(String id, String state, double x, double y, double z) {
        return spawnNpc(id, state, x, y, z, 0, 0, 0);
    }

    public ScriptNpc spawnNpc(String id, String state, double x, double y, double z, float yaw, float pitch, float headYaw) {
    ...
    public ScriptNpc spawnNpc(String id, String state, ScriptVector pos) {
        return spawnNpc(id, state, pos.x, pos.y, pos.z, 0, 0, 0);
    }

    public ScriptNpc spawnNpc(String id, String state, ScriptVector pos, ScriptVector rot) {
    ...
    public ScriptEntityItem dropItemStack(ScriptItemStack stack, double x, double y, double z, double mx, double my, double mz) {
    ...
    public ScriptRayTrace rayTrace(double x1, double y1, double z1, double x2, double y2, double z2) {
        return new ScriptRayTrace(RayTracing.rayTraceWithEntity(world, x1, y1, z1, x2, y2, z2));
    }

    public ScriptRayTrace rayTraceBlock(double x1, double y1, double z1, double x2, double y2, double z2) {
    ...
    public void fill(ScriptBlockState block, int x1, int y1, int z1, int x2, int y2, int z2) {
        fill(block, new ScriptBox(x1, y1, z1, x2, y2, z2));
    }

    public void fill(ScriptBlockState block, ScriptVector start, ScriptVector end) {
        fill(block, new ScriptBox(start.x, start.y, start.z, end.x, end.y, end.z));
    }

    public void fill(ScriptBlockState block, ScriptBox box) {
    ...
    @Deprecated
    public void fillTileEntities(int x1, int y1, int z1, int x2, int y2, int z2, ScriptBlockState block, ScriptNBTCompound tileData) {
    ...
    public void clone(int x, int y, int z, int xNew, int yNew, int zNew) {
        ...
        ScriptNBTCompound tile = getTileEntity(x, y, z).getData();
        tile.setInt("x", xNew);
        tile.setInt("y", yNew);
        tile.setInt("z", zNew);
    ...
    public void clone(ScriptBox box, int xNew, int yNew, int zNew) {
        List<Pair<ScriptBlockState, ScriptNBTCompound>> blocks = new ArrayList<>();
        ...
        for (int x = minX; x <= box.maxX; x++) {
            for (int y = minY; y <= box.maxY; y++) {
                for (int z = minZ; z <= box.maxZ; z++) {
                    ScriptTileEntity tileEntity = getTileEntity(x, y, z);
        ...
        for (int x = minX; x <= box.maxX; x++) {
            for (int y = minY; y <= box.maxY; y++) {
                for (int z = minZ; z <= box.maxZ; z++) {
                    Pair<ScriptBlockState, ScriptNBTCompound> pair = blocks.get(i++);
    ...
    public ScriptItemStack getBlockItem(int x, int y, int z) {
