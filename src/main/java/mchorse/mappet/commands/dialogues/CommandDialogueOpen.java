...
public class CommandDialogueOpen extends CommandDialogueBase {
    ...
    public void executeCommand(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (dialogue.main == null) throw new CommandException("dialogue.empty", id);

        EntityPlayerMP player = getPlayer(server, sender, args[0]);
        DataContext context = new DataContext(player);
        if (args.length > 2) context.parse(String.join(" ", SubCommandBase.dropFirstArguments(args, 2)));
        Mappet.dialogues.open(player, dialogue, new DialogueContext(context));
    }

    @Override
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        return args.length == 1 ? getListOfStringsMatchingLastWord(args, ServerUtils.playerNames(server))
