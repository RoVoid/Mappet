...
public class CommandDataClear extends MappetCommandBase {
    public static void clear(EntityPlayerMP player, boolean inventory) {
        ...
        if (inventory) player.inventory.clear();
    }
    ...
    public void executeCommand(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        for (EntityPlayerMP player : server.getPlayerList().getPlayers()) {
            ...
            ICharacter character = Character.get(player);
            if (character != null) character.updateLastClear(Instant.now());
        }

        getL10n().info(sender, inventory ? "data.cleared_inventory" : "data.cleared");
    }

    @Override
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        return args.length == 1 ? getListOfStringsMatchingLastWord(args, McCommandBase.BOOLEANS)
