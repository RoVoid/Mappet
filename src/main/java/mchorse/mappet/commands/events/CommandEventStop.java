...
public class CommandEventStop extends CommandEventBase {
    ...
    public void executeCommand(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        int removed = CommonProxy.playerSessionHandler.getExecutableScheduler().removeExecutables(args[0]);
        Mappet.l10n.success(sender, "events.stopped", removed, args[0]);
    }

    @Override
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        return args.length == 1 ? getListOfStringsMatchingLastWord(args, CommonProxy.playerSessionHandler.getExecutableScheduler().getIds())
