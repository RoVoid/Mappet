...
public class GuiConditionOverlayPanel extends GuiEditorOverlayPanel<AbstractConditionBlock> {
    ...
    private void copyCondition() {
        ...
        tag.setString("Type", CommonProxy.getConditionBlocks().type(block));
    ...
    protected void fillData(AbstractConditionBlock block) {
        ...
        try {
            this.editor.add(
                    (GuiAbstractConditionBlockPanel) PANELS.get(block.getClass()).getConstructors()[0].newInstance(this.mc, this, block));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static class GuiAbstractBlockListElement extends GuiListElement<AbstractConditionBlock> {
        ...
        protected void drawElementPart(AbstractConditionBlock element, int i, int x, int y, boolean hover, boolean selected) {
            int color = CommonProxy.getConditionBlocks().color(element);
            ...
            if (element.not) {
                GuiDraw.drawTextBackground(this.font, "!", x + 6, y + this.scroll.scrollItemSize / 2 - this.font.FONT_HEIGHT / 2, 0xffffff,
                        ColorUtils.HALF_BLACK, 2);
            }
