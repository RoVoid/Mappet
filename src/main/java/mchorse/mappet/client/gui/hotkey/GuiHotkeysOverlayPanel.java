...
public class GuiHotkeysOverlayPanel extends GuiOverlayPanel {
    ...
    public GuiHotkeysOverlayPanel(Minecraft mc, Hotkeys hotkeys) {
        ...
        name.tooltip(IKey.lang("mappet.gui.nodes.event.hotkeys.name"), Direction.TOP);
        ...
        mode = new GuiButtonElement(mc, IKey.EMPTY, (b) -> {
            ...
            mode.label = IKey.lang("mappet.gui.nodes.event.hotkeys.mode." + hotkey.mode.toString());
        });
        mode.tooltip(IKey.lang("mappet.gui.nodes.event.hotkeys.mode_tooltip"));
        ...
        editor.add(name, Elements.label(IKey.lang("mappet.gui.nodes.event.hotkeys.key")), key, mode);
        editor.add(trigger.marginTop(12));
        editor.add(Elements.label(IKey.lang("mappet.gui.nodes.event.hotkeys.enabled")).marginTop(12), enabled);

        content.add(editor, list);

        pickHotkey(hotkeys.keys.isEmpty() ? null : hotkeys.keys.values().iterator().next(), true);
    }
    ...
    private void renameHotkey(String newName) {
        ...
        if (!newName.isEmpty() && hotkeys.keys.containsKey(newName) && hotkeys.keys.get(newName) != hotkey) {
            ...
            name.field.setCursorPositionEnd();
            return;
        }
    ...
    private void removeHotkey() {
        ...
        list.getList().remove(list.getIndex());
        list.update();
        pickHotkey(hotkeys.keys.isEmpty() ? null : hotkeys.keys.values().iterator().next(), true);
    }
